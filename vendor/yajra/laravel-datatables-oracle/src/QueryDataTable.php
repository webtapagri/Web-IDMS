...
class QueryDataTable extends DataTableAbstract
{
    ...
    public function count()
    {
		...
		$cName = 'countdttbl_'.clean($builder->toSql()) . clean(json_encode($builder->getBindings()));
		
		if(config('cache.default') == 'redis'){
			$cek = Cache::tags(AccessRight::plants())->has($cName);
		}else{
			$cek = Cache::has($cName);
		}
		...
		if ($cek){
			if(config('cache.default') == 'redis'){
				$value = Cache::tags(AccessRight::plants())->get($cName);
			}else{
				$value = Cache::get($cName);
			}
			
			$LOG = 'Ini data dari CACHE - datatables count()';
		} else {
			if(config('cache.default') == 'redis'){
				$value = Cache::tags(AccessRight::plants())->remember($cName, config('app.cache_query_time'), function () use($table, $builder) {
					return $this->connection->table($table)
									->setBindings($builder->getBindings())
									->count();
				});
			}else{
				$value = Cache::remember($cName, config('app.cache_query_time'), function () use($table, $builder) {
					return $this->connection->table($table)
									->setBindings($builder->getBindings())
									->count();
				});
    ...
    {
		$cName = clean($this->query->toSql()) . clean(json_encode($this->query->getBindings()));
		...
		if ($cek){
			...
			$LOG = 'Ini data dari CACHE - datatables results()';
		} else {
			if(config('cache.default') == 'redis'){
				$value = Cache::tags(AccessRight::plants())->remember($cName, config('app.cache_query_time'), function () {
					return $this->query->get();
				});
			}else{
				$value = Cache::remember($cName, config('app.cache_query_time'), function () {
					return $this->query->get();
				});
			}
			
			$LOG = 'Ini data dari DATABASE - datatables results()';
		}	
